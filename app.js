...
function doesExistNumber(str) {
    return !/\d/.test(str);
}

function validate(firstname, lastname, age, email) {
    if (firstname.value.length < 3 || !doesExistNumber(firstname.value)) {
        alert('Ism 3 ta harfdan iborat bo‘lishi kerak.');
        firstname.focus();
        return false;
    }

    if (lastname.value.length < 3 || !doesExistNumber(lastname.value)) {
        alert('Familiya 3 ta harfdan iborat bo‘lishi kerak.');
        lastname.focus();
        return false;
    }
